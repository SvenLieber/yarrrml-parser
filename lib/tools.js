...
async function compareY2RFiles(ymlPaths, ttlPath, options, cb) {
  const yamls = [];
  ...
  await compareY2RData(yamls, ttl, options, cb);
}

async function compareY2R2RFiles(ymlPath, ttlPath, options, cb) {
  const yaml = fs.readFileSync(ymlPath, 'utf8');
  const ttl = fs.readFileSync(ttlPath, 'utf8');
  await compareY2R2RData(yaml, ttl, options, cb);
}

async function compareY2RData(yaml, ttl, options, cb) {
  const y2r = new convertYAMLtoRML(options);
  const yamlQuads = await y2r.convert(yaml);
  const rmlQuads = [];
...
async function compareY2R2RData(yaml, ttl, options, cb) {
