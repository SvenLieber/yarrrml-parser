...
describe('YARRRML to RML', function () {
  ...
  it.skip('works for dbpedia', function () {
    ...
    assert.deepStrictEqual(jsonldObj, shouldBeObj);
  });

  it('Fails for an empty file', async function () {
    await expectInvalidYARRRMLError(path.resolve(__dirname, '../test/trivial/null.yml'));
  });
  ...
  it('works for prefix expansion', function (done) {
    work('prefix-expand/mapping.yml', 'prefix-expand/mapping.rml.ttl', done);
  });
  ...
  it('condition on mapping with blank node', function (done) {
    work('condition-on-mapping-with-blanknode/mapping.yarrrml', 'condition-on-mapping-with-blanknode/mapping.rml.ttl', done);
  });
  ...
  it('multiple input files', function (done) {
    workMultipleInputFiles(['multiple-input-files/mapping-1.yarrrml', 'multiple-input-files/mapping-2.yarrrml'],
      'multiple-input-files/mapping.rml.ttl', done);
  });

  describe('external references', function () {
    doTestCase('external-references/1');
    doTestCase('external-references/2');
    doTestCase('external-references/3');
    doTestCase('external-references/4');
    doTestCase('external-references/5');
   });
});
...
/**
 * This method converts a YARRRML document to RML rules and expects an error.
 * @param yarrrmlPath - The path to a YARRRML document.
 * @param options - The options of the converter.
 * @returns {Promise<void>}
 */
async function expectInvalidYARRRMLError(yarrrmlPath, options = null) {
  const y2r = new convertYAMLtoRML(options);
  const yaml = await fs.readFile(yarrrmlPath, 'utf8');
  let error;

  try {
    await y2r.convert(yaml);
  } catch (e) {
    error = e ;
  }
